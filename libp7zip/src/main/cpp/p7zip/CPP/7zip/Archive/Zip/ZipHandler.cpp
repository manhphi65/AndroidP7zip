...
namespace NArchive {
    namespace NZip {
        ...
        static const char *const kMethods[] =
                {
                        "Store", "Shrink", "Reduced1", "Reduced2", "Reduced3", "Reduced4",
                        "Implode", "Tokenizing", "Deflate", "Deflate64", "PKImploding"
                };
        ...
        STDMETHODIMP CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT *value) {
            COM_TRY_BEGIN
                ...
                switch (propID) {
                    case kpidBit64:
                        if (m_Archive.IsZip64) prop = m_Archive.IsZip64;
                        break;
                    case kpidComment:
                        if (m_Archive.ArcInfo.Comment.Size() != 0)
                            prop = MultiByteToUnicodeString(
                                    BytesToString(m_Archive.ArcInfo.Comment), CP_ACP);
                        break;
                    ...
                    case kpidTotalPhySize:
                        if (m_Archive.IsMultiVol) prop = m_Archive.Vols.GetTotalSize();
                        break;
                    case kpidVolumeIndex:
                        if (m_Archive.IsMultiVol) prop = (UInt32) m_Archive.Vols.StartVolIndex;
                        break;
                    case kpidIsVolume:
                        if (m_Archive.IsMultiVol) prop = true;
                        break;
                    case kpidNumVolumes:
                        if (m_Archive.IsMultiVol) prop = (UInt32) m_Archive.Vols.Streams.Size();
                        break;
        ...
        STDMETHODIMP CHandler::GetProperty(UInt32 index, PROPID propID, PROPVARIANT *value) {
            COM_TRY_BEGIN
                ...
                switch (propID) {
                    case kpidPath: {
                        UString res;
                        item.GetUnicodeString(res, item.Name, false, _forceCodePage,
                                              _specifiedCodePage);
                    ...
                    case kpidComment: {
                        if (item.Comment.Size() != 0) {
                            UString res;
                            item.GetUnicodeString(res, BytesToString(item.Comment), true,
                                                  _forceCodePage, _specifiedCodePage);
                            prop = res;
                        }
                        break;
                    }
                    ...
                    case kpidCRC:
                        if (item.IsThereCrc()) prop = item.Crc;
                        break;
                    ...
                    case kpidMethod: {
                        ...
                        if (item.IsEncrypted()) {
                            if (id == NFileHeader::NCompressionMethod::kWzAES) {
                                ...
                                if (extra.GetWzAes(aesField)) {
                                    char s[16];
                                    s[0] = '-';
                                    ConvertUInt32ToString(((unsigned) aesField.Strength + 1) * 64,
                                                          s + 1);
                                    m += s;
                                    id = aesField.Method;
                                }
                                ...
                                if (extra.GetStrongCrypto(f)) {
                                    const char *s = FindNameForId(k_StrongCryptoPairs,
                                                                  ARRAY_SIZE(k_StrongCryptoPairs),
                                                                  f.AlgId);
                                    if (s)
                                        m += s;
                                    else {
...

                        {
                            ...
                            if (id < ARRAY_SIZE(kMethods))
                                ...
                                s = FindNameForId(k_MethodIdNamePairs,
                                                  ARRAY_SIZE(k_MethodIdNamePairs), id);
                                if (!s) {
        ...
        STDMETHODIMP CHandler::Open(IInStream *inStream,
            ...
            COM_TRY_BEGIN
                try {
                    Close();
                    HRESULT res = m_Archive.Open(inStream, maxCheckStartPosition, callback,
                                                 m_Items);
                    if (res != S_OK) {
                        m_Items.Clear();
                        m_Archive.ClearRefs();
        ...
        HRESULT CZipDecoder::Decode(
                ...
                if (getTextPassword) {
                    ...
                    if (password) {
                        if (wzAesMode || pkAesMode) {
                            charPassword = UnicodeStringToMultiByte((const wchar_t *) password,
                                                                    CP_ACP);
                            /*
                            for (unsigned i = 0;; i++)
                            {
            ...
            if (m == methodItems.Size()) {
                ...
                if (id == NFileHeader::NCompressionMethod::kStored)
                    ...
                    RINOK(CreateCoder(EXTERNAL_CODECS_LOC_VARS
                                  szMethodID, false, mi.Coder));
                    LOGD("Call Here!");
                    if (mi.Coder == 0) {
                        res = NExtract::NOperationResult::kUnsupportedMethod;
                        LOGD("Call Here!");
            ...
            LOGD("Call here");
            {
                ...
                if (item.IsEncrypted()) {
                    ...
                    if (wzAesMode) {
                        ...
                        if (result == S_OK) {
                            ...
                            Byte v1 = _zipCryptoDecoderSpec->_header[NCrypto::NZip::kHeaderSize -
                                                                     1];
                            Byte v2 = (Byte) (item.HasDescriptor() ? (item.Time >> 8) : (item.Crc
                                    >> 24));
                ...
                if (result == S_OK)
                    result = coder->Code(inStreamNew, outStream, NULL, &item.Size,
                                         compressProgress);
