...
*/
        ...
        STDMETHODIMP CHandler::Open(IInStream *stream, const UInt64 *,
...
HRESULT
Archive_GetItemBoolProp(IInArchive *arc, UInt32 index, PROPID propID, bool &result) throw() {
    NCOM::CPropVariant prop;
    result = false;
...
static HRESULT
Archive_GetArcProp_UInt(IInArchive *arc, PROPID propid, UInt64 &result, bool &defined) {
    defined = false;
    NCOM::CPropVariant prop;
    RINOK(arc->GetArchiveProperty(propid, &prop));
    switch (prop.vt) {
...
static HRESULT
Archive_GetArcProp_Int(IInArchive *arc, PROPID propid, Int64 &result, bool &defined) {
    defined = false;
    NCOM::CPropVariant prop;
    RINOK(arc->GetArchiveProperty(propid, &prop));
    switch (prop.vt) {
...
static HRESULT
Archive_GetItem_Size(IInArchive *archive, UInt32 index, UInt64 &size, bool &defined) {
    NCOM::CPropVariant prop;
    defined = false;
    size = 0;
...
static const char *const k_Formats_with_simple_signuature[] =
        {
                "7z", "xz", "rar", "bzip2", "gzip", "cab", "wim", "rpm", "vhd", "xar"
        };
...
static bool IsNewStyleSignature(const CArcInfoEx &ai) {
    ...
    return IsNameFromList(ai.Name, k_Formats_with_simple_signuature,
                          ARRAY_SIZE(k_Formats_with_simple_signuature));
}
...
public:
    ...
    MY_QUERYINTERFACE_BEGIN2(IArchiveOpenCallback)MY_QUERYINTERFACE_ENTRY(
...
HRESULT
CArc::PrepareToOpen(const COpenOptions &op, unsigned formatIndex, CMyComPtr<IInArchive> &archive) {
...
static HRESULT
ReadParseItemProps(IInArchive *archive, const CArcInfoEx &ai, NArchive::NParser::CParseItem &pi) {
...
STDMETHODIMP
CExtractCallback_To_OpenCallback::GetStream(UInt32 /* index */, ISequentialOutStream **outStream,
...
static int FindFormatForArchiveType(CCodecs *codecs, CIntVector orderIndices, const char *name) {
    FOR_VECTOR (i, orderIndices)if (StringsAreEqualNoCase_Ascii(
                codecs->Formats[orderIndices[i]].Name, name))
            return i;
    return -1;
}
...
HRESULT CArc::OpenStream2(const COpenOptions &op) {
    ...
    {
        FOR_VECTOR(i, op.codecs->Formats)isMainFormatArr[i] = false;
    }
            ...
            if (numFinded == 0 || IsExeExt(extension)) {
                ...
                if (i >= 0) {
                    ...
                    if (processedSize >= 16) {
                        ...
                        if (TestSignature(buf, kRarHeader, 7) && buf[9] == 0x73 &&
                            (buf[10] & 1) != 0) {
                ...
                */
                ...
                MakeCheckOrder(op.codecs, orderIndices, numFinded, orderIndices2, byteBuffer,
                               processedSize);
    ...
    {
        FOR_VECTOR(i, op.codecs->Formats)skipFrontalFormat[i] = false;
    }
...

    {
        ...
        {
            size_t bufSize = 1
                    << 20; // it must be larger than max signature offset or IsArcFunc offset ((1 << 19) + x for UDF)
            if (bufSize > fileSize) {
        ...
        for (i = 0; i < sortedFormats.Size(); i++) {
            ...
            HRESULT result;
            {
                ...
                */
                result = OpenArchiveSpec(archive, !mode.CanReturnArc, op.stream, &searchLimit,
                                         op.callback, extractCallback_To_OpenCallback);
            }
            ...
            */
            ...
            if (mode.CanReturnArc) {
                bool isMainFormat = isMainFormatArr[(unsigned) FormatIndex];
                const COpenSpecFlags &specFlags = mode.GetSpec(isForced, isMainFormat,
                                                               isUnknownExt);
                bool openCur = false;
    ...
    CUIntVector sig2arc; // signatureIndex to formatIndex;
    {
        ...
        FOR_VECTOR (i, op.codecs->Formats) {
            ...
            sum += sigs.Size();
            FOR_VECTOR (k, sigs)sig2arc.Add(i);
        }
    }

    {
        ...
        if (op.callback) {
            ...
            openCallback_Offset_Spec->Callback.QueryInterface(IID_IArchiveOpenVolumeCallback,
                                                              &openCallback_Offset_Spec->OpenVolumeCallback);
#ifndef _NO_CRYPTO
            openCallback_Offset_Spec->Callback.QueryInterface(IID_ICryptoGetTextPassword,
                                                              &openCallback_Offset_Spec->GetTextPassword);
#endif
        }
            ...
            for (;;) {
                ...
                if (ai.IsArcFunc && startArcPos >= bufPhyPos) {
                    ...
                    if (offsetInBuf < bytesInBuf) {
                        UInt32 isArcRes = ai.IsArcFunc(byteBuffer + offsetInBuf,
                                                       bytesInBuf - offsetInBuf);
                        if (isArcRes == k_IsArc_Res_NO)
                            continue;
                        if (isArcRes == k_IsArc_Res_NEED_MORE && endOfFile)
                            continue;
        ...
        */
        ...
        if (mode.CanReturnParser) {
            bool returnParser = (handlerSpec->_items.Size() ==
...
HRESULT CArchiveLink::Open(COpenOptions &op) {
    ...
    for (;;) {
        ...
        */
        ...
        RINOK(Archive_GetItemBoolProp(arc.Archive, mainSubfile, kpidZerosTailIsAllowed,
                                      zerosTailIsAllowed));


        if (op.callback) {
            CMyComPtr<IArchiveOpenSetSubArchiveName> setSubArchiveName;
            op.callback->QueryInterface(IID_IArchiveOpenSetSubArchiveName,
